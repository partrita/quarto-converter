...
def process_pdf(file_path, output_dir, images_dir):
    ...
    for i in range(len(toc) - 1):
        ...
        for p_idx in range(start_idx, min(end_idx, doc.page_count)):
            ...
            # Extract Images
            # Logic: If we have extracted significant text, skip full-page background images
            # which are likely the source of the OCR text.
            ...
            for img_info in image_list:
                ...
                # Check image dimensions/position if possible
                # In PyMuPDF, get_image_rects returns the rects where the image is displayed
                img_rects = page.get_image_rects(xref)
                is_full_page = False
                if img_rects:
                    r = img_rects[0]
                    # If image covers more than 80% of the page area, consider it full page
                    if (r.width * r.height) > (page_rect.width * page_rect.height * 0.8):
                        is_full_page = True
                
                # Skip if it's a full-page image and we already have text (OCR case)
                if is_full_page and has_text:
                    print(f"Skipping background image on page {p_idx+1} (OCR text already extracted)")
                    continue
                ...
                # Insert reference into markdown
                chapter_md += f"\n\n![](images/{img_name})\n\n"
...
def update_quarto_yml(yml_path, new_chapters):
    if not yml_path.exists():
        ...
        config = {"project": {"type": "book"}, "book": {"title": "Converted Book", "chapters": ["index.qmd"]}}
    else:
    ...
    # Ensure index.qmd is first
    if "index.qmd" not in config["book"]["chapters"]:
        config["book"]["chapters"].insert(0, "index.qmd")
...
def create_index_qmd(output_dir, title, author, chapters):
    ...
    content += "This book was automatically converted from a PDF document.\n\n"
    content += "## Table of Contents (Generated)\n\n"
    for i, ch in enumerate(chapters):
        # Human-readable label
        label = ch.replace(".qmd", "").replace("_", " ").title()
        content += f"- [{label}]({ch})\n"
...
def main():
    ...
    if ext == ".pdf":
        ...
        # Simplified metadata extraction for EPUB could be added here
    ...
    if chapters:
        create_index_qmd(output_path, title, author, chapters)
